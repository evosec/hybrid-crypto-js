...
class Crypt {
    ...
    encryptAESKeyByRSA(publicKey, aesKey){
        publicKey = typeof publicKey === 'string' ? pki.publicKeyFromPem(publicKey) : publicKey;

        // Encrypt random key with all of the public keys
        var encryptedKey = publicKey.encrypt(aesKey, 'RSAES-PKCS1-V1_5');
        let encodedKey = forge.util.encode64(encryptedKey);
