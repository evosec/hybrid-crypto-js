...
class Crypt {
    ...
    encryptByAES(message, aesKey, iv) {

        // Create buffer and cipher
        var buffer = forge.util.createBuffer(message, 'utf8');
        var cipher = forge.cipher.createCipher(AES_STANDARD, aesKey);
        ...
        let encryptedMessage = forge.util.encode64(cipher.output.data);

        // Return encrypted message
        return encryptedMessage;
    }

    encryptAESKeyByRSA(publicKeys, aesKey){
        var self = this;
        ...
        // Map PEM keys to forge public key objects
        publicKeys = publicKeys.map(function(bundledKeys) {
            if (typeof bundledKeys === 'string')
                return pki.publicKeyFromPem(bundledKeys)
            return bundledKeys;
        });
        ...
        let encodedKey;
        publicKeys.forEach(function(publicKey) {
            var encryptedKey = publicKey.encrypt(aesKey, 'RSAES-PKCS1-V1_5');
            var fingerprint = self.fingerprint(publicKey);
            encodedKey = forge.util.encode64(encryptedKey);
            encryptedKeys[fingerprint] = forge.util.encode64(encryptedKey);
        });
    ...
    decrypt(privateKey, encrypted) {
        ...
        console.log("encryptedKey in decryption: "+encryptedKey);
